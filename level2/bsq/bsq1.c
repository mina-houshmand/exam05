...
int validate(const Map *m)
{
    if (!m || m->rows < 1 || m->cols < 1) 
		return 0;   // ✓ At least one line of at least one box

    if (m->empty == m->obs || m->empty == m->fill || m->obs == m->fill) 
		return 0;  // ✓ The three characters must be different
    
	for (int i = 0; i < m->rows; i++)
        for (int j = 0; j < m->cols; j++)
            if (m->grid[i][j] != m->empty && m->grid[i][j] != m->obs)
                return 0;  //✓ The grid must only contain the empty and obstacle characters
    return 1;
}
...
void solve(Map *m)
{
    int R = m->rows;
	int C = m->cols;
	// It will store the size of the largest square that can end at each cell (i, j).
